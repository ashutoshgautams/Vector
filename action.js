...
var TitleScreen2 = function TitleScreen2(title,subtitle,s_subtitle,callback) {
  var up = false;
  this.step = function(dt) {
    if(!Game.keys['fire']) up = true;
    if(up && Game.keys['fire'] && callback) callback();
  ...
  this.draw = function(ctx) {
    ...
    ctx.font = "20px Papyrus";
    var measure2 = ctx.measureText(subtitle);
    ctx.fillText(subtitle,Game.width/2 - measure2.width/2,Game.height/2 + 35);
    ctx.font = "14px Papyrus";
    var measure2 = ctx.measureText(s_subtitle);
    ctx.fillText(s_subtitle,Game.width/2 - measure2.width/2,Game.height/2 + 90);
  };
};
var TitleScreen3 = function TitleScreen3(title,subtitle,callback) {
  var up = false;
  this.step = function(dt) {
    if(!Game.keys['fire']) up = true;
    if(up && Game.keys['fire'] && callback) callback();
...
var Titlespcl = function Titlespcl(title) {
  var up = false;
  this.step = function(dt) {
    if(!Game.keys['fire']) up = true;
    if(up && Game.keys['fire'] && callback) callback();
